...
uint32_t action = 0;
uint32_t random_flag = 0;

uint64_t start_period[NUM_CPUS] = {0};
uint64_t samp_period = 100000;
uint64_t total_prev_access[NUM_CPUS] = {0};
uint64_t total_prev_active_cycle[NUM_CPUS] = {0};
uint64_t obstruction[NUM_CPUS] = {0};
...
void CACHE::handle_fill()
{
	...
	{
		int now_cycle = current_core_cycle[fill_cpu];
		if(now_cycle - start_period[fill_cpu] >= samp_period)
		{
			obstruction_candidate(fill_cpu);
                        start_period[fill_cpu] = now_cycle;
		}
	}
	...
	if (cache_type == IS_LLC)
	{
        		...
        		if(active_cycles.size() >= SET_MAX && active_miss_cycles.size() >= SET_MAX)
        		{
                        ...
                        if (active_hit_cycles_per_core[fill_cpu].find(now) == active_hit_cycles_per_core[fill_cpu].end())
                        {
                                pure_miss_tag = 1;
                               	...
                               	{
                                       	if (MSHR.entry[i].cpu == fill_cpu)
                                       	{
                                               	num++;
                                       	}
                               	}
                               	num = MSHR_occupy[fill_cpu][now];
                               	if(num == 0)
                               	{
                                       	MSHR.entry[mshr_index].pmc = MSHR.entry[mshr_index].pmc + 1.0;
                               	}
                               	else
                               	{
                                       	MSHR.entry[mshr_index].pmc = MSHR.entry[mshr_index].pmc + 1.0/num;
                               	}
                       	}
                       	else
                        {
        ...
        if (cache_type == IS_LLC) {
            action = llc_management_decision(fill_cpu, set, 0, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, 0, MSHR.entry[mshr_index].type, 0, obstruction);
            way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type, action);
        }
        else
	...
	if ((cache_type == IS_LLC) && (way == LLC_WAY)) { // this is a bypass that does not fill the LLC
            ...
            if (cache_type == IS_LLC) {
                llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, 0, MSHR.entry[mshr_index].type, 0, action);
                //stats.llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, 0, MSHR.entry[mshr_index].type, 0, 1, 1, random_flag);
            }
            else
	...
	switch ((int)MSHR.entry[mshr_index].pmc / 50) {
		...
		cpu = 0;
            ...
            // update replacement policy
            if (cache_type == IS_LLC) {
                llc_update_replacement_state(fill_cpu, set, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, block[set][way].full_addr, MSHR.entry[mshr_index].type, 0,  action);
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if (cache_type == IS_LLC) {
                llc_update_replacement_state(writeback_cpu, set, way, block[set][way].full_addr, WQ.entry[index].ip, 0, WQ.entry[index].type, 1, 0);
                //stats.llc_update_replacement_state(writeback_cpu, set, way, block[set][way].full_addr, WQ.entry[index].ip, 0, WQ.entry[index].type, 1, 0, 0, 0);
            }
            else
		  ...
		  if(cache_type == IS_LLC)
		  ...
		  else
		    {
		      ...
		      lower_level->add_rq(&WQ.entry[index]);
                ...
                if (cache_type == IS_LLC) {
                    way = llc_find_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type, 0);
                }
                else
			...
			cpu = 0;
                    ...
                    // update replacement policy
                    if (cache_type == IS_LLC) {
                        llc_update_replacement_state(writeback_cpu, set, way, WQ.entry[index].full_addr, WQ.entry[index].ip, block[set][way].full_addr, WQ.entry[index].type, 0, 0);
                        //stats.llc_update_replacement_state(writeback_cpu, set, way, WQ.entry[index].full_addr, WQ.entry[index].ip, block[set][way].full_addr, WQ.entry[index].type, 0, 0, 0, 0);                    
		    }
                    else
      ...
      if (read_cpu == NUM_CPUS)
      {
		...
		if (RQ.entry[index].type == LOAD) {
			...
			cpu = 0;
                ...
                // update replacement policy
                if (cache_type == IS_LLC) {
		    action = llc_management_decision(read_cpu, set, way, block[set][way].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, 1, obstruction);
		    llc_update_replacement_state(read_cpu, set, way, block[set][way].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, 1, action);
                    //stats.llc_update_replacement_state(read_cpu, set, way, block[set][way].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, 1, 0, 0, 0);
                }
                else
...
void CACHE::handle_prefetch()
{
      ...
      if (prefetch_cpu == NUM_CPUS)
      {
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {            
            ...
            if (way >= 0) { // prefetch hit
                ...
                // update replacement policy
                if (cache_type == IS_LLC) {
 		    action = llc_management_decision(prefetch_cpu, set, way, block[set][way].full_addr, PQ.entry[index].ip, 0, PQ.entry[index].type, 1, obstruction);
                    llc_update_replacement_state(prefetch_cpu, set, way, block[set][way].full_addr, PQ.entry[index].ip, 0, PQ.entry[index].type, 1, action);
                    //stats.llc_update_replacement_state(prefetch_cpu, set, way, block[set][way].full_addr, PQ.entry[index].ip, 0, PQ.entry[index].type, 1, 0, 0, 0);
                }
                else
	...
	{ 
		...
		if(warmup_complete[prefetch_cpu])
            	{
                	miss_count++;
            	}
...
void CACHE::llc_decision_final_stats(uint32_t cpu) {
    stats.llc_decision_final_stats(cpu);
    cout << endl;
}

void CACHE::llc_decision_inform_warmup_complete() {
    stats.llc_decision_inform_warmup_complete();
}

void CACHE::llc_decision_inform_roi_complete(uint32_t cpu) {
    stats.llc_decision_inform_roi_complete(cpu);
}

void CACHE::llc_decision_roi_stats(uint32_t cpu) {
    stats.llc_decision_roi_stats(cpu);
    cout << endl;
}
...
{
    /* boradcast to LLC replacement */
    if (cache_type == IS_LLC)
    {
    	llc_replacement_broadcast_bw(bw_level);
    }

}

void CACHE::broadcast_cal_level(uint32_t cal)
{
    /* boradcast to LLC replacement */
    if (cache_type == IS_LLC)
    {
        llc_replacement_broadcast_cal_level(cal);
    }

}

void  CACHE::obstruction_candidate(uint32_t core)
{
        uint64_t t_mem = 150; // tRP + tRCD + tCAS;

        uint64_t total_current_access = 0;
        uint64_t total_current_active_cycle = 0;

        uint64_t access = 0;
	uint64_t active_cycle = 0;

	total_current_active_cycle = total_active_cycles_per_core[core] + active_cycles_per_core[core].size();
                
	for (uint32_t j = 0; j < NUM_TYPES; j++)
        {
        	total_current_access += sim_access[core][j];
        }

        access = total_current_access - total_prev_access[core];
        active_cycle = total_current_active_cycle - total_prev_active_cycle[core];

	if ((1.0 * active_cycle / access) > t_mem)
        {
        	obstruction[core] = 1;
        }
	else
	...
	total_prev_active_cycle[core] = total_current_active_cycle;
        total_prev_access[core] = total_current_access;
