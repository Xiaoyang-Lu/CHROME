...
#define measure_dram_bw_epoch 2048
#define eq_period 256
...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
    ...
    cout<< "Core_" << cpu << "_" << cache->NAME << "_total_access " << TOTAL_ACCESS << endl
        ...
        << "Core_" << cpu << "_" << cache->NAME << "_total_miss_isca " << TOTAL_MISS << endl
...
//	cout << " pmc_1300_above: " << cache->pmc_1300_above  << endl;
	...
	cout << " MISS_total: " << cache->miss_total << endl;
        ...
        cout << "Replacement_count: " << cache->re_count << endl;
        cout << "Bypassing_count: " << cache->by_count << endl;
        cout << "Accurate_decision: " << cache->accurate << endl;
        cout << "Inaccurate_decision: " << cache->inaccurate << endl;
        cout << "pcs: " << cache->pcs.size() << endl;
        cout << "addrs: " << cache->addrs.size() << endl;
        cout << "pages: " << cache->pages.size() << endl;
        cout << "offsets: " << cache->offsets.size() << endl;
        cout << "pcs+offsets: " << cache->pc_offset.size() << endl;
        cout << "pcs+pages: " << cache->pc_page.size() << endl;
        cout << "last_4_pcs: " << cache->pc_path.size() << endl;
        cout << "last_4_pcs+offset " << cache->pc_path_offset.size() << endl;
        cout << "last_4_pcs+page " << cache->pc_path_page.size() << endl;
        cout << "total " << cache->total << endl;
	cout << "max_cal " << cache->max_cal << endl;
	for(int i = 0; i < 16; i++)
	{
		cout << "wictim_lru_position_" << i << ": "  << cache->victim_lru[i]  << endl;
	}
...
void print_dram_stats()
{
    ...
    for(uint32_t index = 0; index < LLC_CAL_LEVELS; ++index)
    {
        cout << "LLC_cal_level_" << index << " " << uncore.LLC.cal_level_hist[index] << endl;
    }	
...
void finish_warmup()
{
    ...
    uncore.LLC.llc_decision_inform_warmup_complete();
}
...
int main(int argc, char** argv)
{
    ...
    cout << "llc_initialize_replacement complete " << endl;
    uncore.LLC.llc_prefetcher_initialize();
    //uncore.LLC.llc_initialize_bypassing(zero_init);
    ...
    while (run_simulation) {
		...
		uncore.LLC.llc_decision_inform_roi_complete(i);
...
/*
	if(uncore.cycle >= uncore.DRAM.next_bw_measure_cycle)
	{
		uint64_t this_epoch_enqueue_count = uncore.DRAM.rq_enqueue_count - uncore.DRAM.last_enqueue_count;
		uint64_t this_epoch_miss_count = uncore.LLC.miss_count - uncore.LLC.last_miss_count;
		
		uncore.DRAM.epoch_enqueue_count = (uncore.DRAM.epoch_enqueue_count/2) + this_epoch_enqueue_count;
		uncore.LLC.epoch_miss_count = (uncore.LLC.epoch_miss_count/2) + this_epoch_miss_count;
		...
		uncore.LLC.cal = ((float)uncore.LLC.epoch_miss_count/uncore.DRAM.epoch_enqueue_count); 
			
		if(uncore.DRAM.epoch_enqueue_count != 0 && uncore.LLC.cal > uncore.LLC.max_cal)
		{
			uncore.LLC.max_cal = uncore.LLC.cal;
		}

		if(uncore.DRAM.epoch_enqueue_count == 0)
		{
			uncore.LLC.cal = 0;
		}		


		if(uncore.LLC.cal <= 1) 	uncore.LLC.cal_level = 0;
		else if(uncore.LLC.cal <= 1.3) 	uncore.LLC.cal_level = 1;
		else if(uncore.LLC.cal <= 5) 	uncore.LLC.cal_level = 2;
		else			     	uncore.LLC.cal_level = 3;
	
		uncore.DRAM.last_enqueue_count = uncore.DRAM.rq_enqueue_count;
		uncore.DRAM.next_bw_measure_cycle = uncore.cycle + measure_dram_bw_epoch;
		uncore.DRAM.total_bw_epochs++;
		uncore.DRAM.bw_level_hist[uncore.DRAM.bw]++;
		uncore.LLC.broadcast_bw(uncore.DRAM.bw);
		

		uncore.LLC.last_miss_count = uncore.LLC.miss_count;
		uncore.LLC.broadcast_cal_level(uncore.LLC.cal_level);
		uncore.LLC.cal_level_hist[uncore.LLC.cal_level]++;
...
*/
	...
	if(this_epoch_enqueue_count > eq_period)
	{
		uint64_t this_epoch_miss_count = uncore.LLC.miss_count - uncore.LLC.last_miss_count;		

		uncore.LLC.cal = ((float)this_epoch_miss_count/this_epoch_enqueue_count);
		if(this_epoch_enqueue_count == 0)
                {
                        uncore.LLC.cal = 0;
                }
                ...
                else if(uncore.LLC.cal <= 2)    uncore.LLC.cal_level = 2;
                else                            uncore.LLC.cal_level = 3;
	...
	uint64_t LLC_MSHR_size_per_cycle[NUM_CPUS];
        for(uint32_t i = 0; i < NUM_CPUS; i++)
        {
		LLC_MSHR_size_per_cycle[i] = 0;
	}
	for(uint32_t i = 0; i < uncore.LLC.MSHR.SIZE; i++)
        {
		uint32_t temp_cpu = uncore.LLC.MSHR.entry[i].cpu;
		if(temp_cpu < NUM_CPUS)
		{
        		LLC_MSHR_size_per_cycle[temp_cpu]++;
		}
	}	

        for(uint32_t i = 0; i < NUM_CPUS; i++)
        {
		uncore.LLC.MSHR_occupy[i].insert(pair<uint64_t, uint32_t> (uncore.cycle, LLC_MSHR_size_per_cycle[i])); 
		//if(LLC_MSHR_size_per_cycle[i] != 0)
		//{
		//	cout << "cycle: " << uncore.cycle << endl;
		//	cout << "core: " << i << endl;
		//	cout << "occupy: " << uncore.LLC.MSHR_occupy[i][uncore.cycle] << endl; 
		//}

		if(uncore.LLC.MSHR_occupy[i].size() >= SET_MAX)
                {
                        std::map<uint64_t, uint32_t>::iterator it = uncore.LLC.MSHR_occupy[i].begin();
                        uncore.LLC.MSHR_occupy[i].erase(it);
                }
	}
...
/*
	...
	for(uint32_t i = 0; i < NUM_CPUS; i++)
	{
		...
		{	
			cout << "core: " << i << endl;
			cout << "occupy: " << LLC_MSHR_size_per_cycle[i] << endl;
		}
	}
*/
...
#endif
            uncore.LLC.llc_decision_final_stats(i);
    ...
    for (uint32_t i=0; i<NUM_CPUS; i++) {
        ooo_cpu[i].L1D.l1d_prefetcher_final_stats();
        ooo_cpu[i].L2C.l2c_prefetcher_final_stats();
	uncore.LLC.llc_decision_roi_stats(i); 
